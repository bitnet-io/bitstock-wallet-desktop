...
#include "atomicdex/services/price/cboe/cboe.provider.hpp"

namespace
{
    //! Using namespace
    using namespace std::chrono_literals;
    using namespace atomic_dex::cboe::api;
...
{
    cboe_provider::cboe_provider(entt::registry& registry, ag::ecs::system_manager& system_manager)  :
        system(registry), m_system_manager(system_manager)
    {
        SPDLOG_INFO("cboe_provider created");
        disable();
        dispatcher_.sink<mm2_started>().connect<&cboe_provider::on_mm2_started>(*this);
        dispatcher_.sink<coin_enabled>().connect<&cboe_provider::on_coin_enabled>(*this);
        dispatcher_.sink<coin_disabled>().connect<&cboe_provider::on_coin_disabled>(*this);
    }

    cboe_provider::~cboe_provider() 
    {
        SPDLOG_INFO("cboe_provider destroyed");
        dispatcher_.sink<mm2_started>().disconnect<&cboe_provider::on_mm2_started>(*this);
        dispatcher_.sink<coin_enabled>().disconnect<&cboe_provider::on_coin_enabled>(*this);
        dispatcher_.sink<coin_disabled>().disconnect<&cboe_provider::on_coin_disabled>(*this);
    }
} // namespace atomic_dex
...
namespace atomic_dex
{
    ...
    TAnswer
    cboe_provider::get_infos(const std::string& ticker, const TRegistry& registry, TLockable& mutex) const 
    {
...
{
    void
    cboe_provider::verify_idx(t_ref_count_idx idx, uint16_t target_size, const std::vector<std::string>& tickers)
    {
        if (idx != nullptr)
        {
    ...
    void
    cboe_provider::generic_post_verification(std::shared_mutex& mtx, TContainer& container, std::string&& ticker, TAnswer&& answer, Args... args)
    {
        {
    ...
    void
    cboe_provider::generic_rpc_cboe_process(
        ...
        Args... args)
    {
        const auto answer_functor = [this, &mtx, &container, functor = std::forward<TExecutorFunctor>(functor), request, ticker = std::move(ticker),
            ...
            if (answer.rpc_result_code == static_cast<web::http::status_code>(antara::app::http_code::too_many_requests))
            {
                ...
                generic_rpc_cboe_process<TAnswer>(request, std::move(ticker), mtx, container, std::forward<TExecutorFunctor>(functor), std::move(args)...);
            }
            else
            {
...
namespace atomic_dex
{
    ...
    void
    cboe_provider::process_provider(const coin_config_t& current_coin, Args... args)
    {
        const price_converter_request request{.base_currency_id = current_coin.cboe_id, .quote_currency_id = "usd-us-dollars"};
        generic_rpc_cboe_process<t_price_converter_answer>(
    ...
    template <typename... Args>
    void
    cboe_provider::process_ticker_infos(const coin_config_t& current_coin, Args... args)
    {
        const ticker_infos_request request{.ticker_currency_id = current_coin.cboe_id, .ticker_quotes = {"USD", "EUR", "BTC"}};
        generic_rpc_cboe_process<ticker_info_answer>(
    ...
    template <typename... Args>
    void
    cboe_provider::process_ticker_historical(const coin_config_t& current_coin, Args... args)
    {
        const ticker_historical_request request{.ticker_currency_id = current_coin.cboe_id, .interval = "2h"};
        generic_rpc_cboe_process<ticker_historical_answer>(
...
{
    void
    cboe_provider::on_mm2_started([[maybe_unused]] const mm2_started& evt) 
    {
        update_ticker_and_provider();
    }

    void
    cboe_provider::on_coin_enabled(const coin_enabled& evt) 
    {
        SPDLOG_INFO("{} enabled, retrieve cboe infos", fmt::format("{}", fmt::join(evt.tickers, ", ")));
    ...
    void
    cboe_provider::on_coin_disabled(const coin_disabled& evt) 
    {
        SPDLOG_INFO("{} disabled, removing from cboe provider", evt.ticker);
...
{
    void
    cboe_provider::update_ticker_and_provider()
    {
        ...
        {
            if (current_coin.cboe_id == "test-coin")
            {
