...
//! Project Headers
#include "atomicdex/api/cboe/cboe.hpp"
...
{
    //! Constants
//    constexpr const char* g_cboe_endpoint = "https://api.coinpaprika.com/v1/";
//    constexpr const char* g_cboe_endpoint = "https://cdn.cboe.com/api/global/delayed_quotes/options/F.json";
//    constexpr const char* g_cboe_endpoint = "https://cdn.cboe.com/api/global";
    constexpr const char* g_cboe_endpoint = "https://cdn.cboe.com/api/global/delayed_quotes/";
    web::http::client::http_client_config g_cboe_cfg{[]() {
      web::http::client::http_client_config cfg;
      cfg.set_validate_certificates(false);
      cfg.set_timeout(std::chrono::seconds(5));
      return cfg;
    }()};
    t_http_client_ptr     g_cboe_client   = std::make_unique<web::http::client::http_client>(FROM_STD_STR(g_cboe_endpoint), g_cboe_cfg);
} // namespace
...
//            evt.price                    = std::to_string(j.at("price").get<double>());
            evt.price                    = std::to_string(j.at("current_price").get<double>());
        ...
        async_price_converter(const price_converter_request& request)
        {
            ...
            return g_cboe_client->request(req);
        }
...
//            auto url                   = "/options/"s + ticker_id + "?quotes="s;
            auto url                   = "/options/"s + ticker_id;
            ...
            SPDLOG_INFO("url: {}", TO_STD_STR(g_cboe_client->base_uri().to_string()) + url);
            req.set_request_uri(FROM_STD_STR(url));
            return g_cboe_client->request(req);
        }
...
//            const auto url                          = "/tickers/"s + ticker_id + "/historical?start="s + std::to_string(timestamp) + "&interval="s + interval;
            const auto url                          = "/options/"s + ticker_id;
            SPDLOG_INFO("url: {}", url);
            req.set_request_uri(FROM_STD_STR(url));
            return g_cboe_client->request(req);
        }
