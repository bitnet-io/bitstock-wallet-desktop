...
//! Project Headers
#include "atomicdex/api/komodo_prices/cboe.prices.hpp"
...
//https://cdn.cboe.com/api/global/delayed_quotes/options/F.json
    constexpr const char*                 g_komodo_prices_endpoint = "https://cdn.cboe.com/";
    constexpr const char*                 g_komodo_prices_endpoint_fallback = "https://cdn.cboe.com/";
   ...
   //     x.sparkline_provider     = j.at("sparkline_provider").get<provider>();
     //   x.change_24_h            = j.at("change_24h").get<std::string>();
       // x.change_24_h_provider   = j.at("change_24h_provider").get<provider>();
        ...
        x.ticker                 = j.at("symbol").get<std::string>();
        x.last_price             = j.at("current_price").get<std::string>();
        x.last_updated           = j.at("last_trade_time").get<std::string>();
        x.price_provider         = j.at("security_type").get<provider>();
...
//        std::string endpoint = fallback ? "api/v2/tickers?expire_at=21600" : "api/v3/prices/tickers_v2?expire_at=21600";
        std::string endpoint = fallback ? "api/global/delayed_quotes/options/F.json" : "api/global/delayed_quotes/options/F.json";
        if (fallback)
        {
